...
#define NCHIPS 8          //max number of LARASIC to configure
      ...
      switch (Command)
      {
        case 'C':
          ...
          if (strcmp(Register, "STS") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_STS);
                if (Bit > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_STS;
              }
          }
          else if (strcmp(Register, "SNC") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_SNC);
                if (Bit > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_SNC;
              }
          }
          else if (strcmp(Register, "SG") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_SG0);
                if ((Bit & 0x1) > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_SG0;
              }
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_SG1);
                if ((Bit & 0x2) > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_SG1;
              }
          }
          else if (strcmp(Register, "ST") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_ST0);
                if ((Bit & 0x1) > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_ST0;
              }
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_ST1);
                if ((Bit & 0x2) > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_ST1;
              }
          }
          else if (strcmp(Register, "SDC") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_SDC);
                if (Bit > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_SDC;
              }
          }
          else if (strcmp(Register, "SBF") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++) for (int ch = 0; ch < 16; ch++)  {
                REG_Ch[i][ch] = REG_Ch[i][ch] & (~C_SBF);
                if (Bit > 0) REG_Ch[i][ch] = REG_Ch[i][ch] | C_SBF;
              }
          }
        ...
        case 'G':
          ...
          if (strcmp(Register, "S16") == 0)
          {
            ...
            for (int i = 0; i < NCHIPS; i++)  {
              REG_Global[i] = REG_Global[i] & (~G_S16);
              if (Bit > 0) REG_Global[i] = REG_Global[i] | G_S16;
            }
          }
          else if (strcmp(Register, "STB") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++)  {
              REG_Global[i] = REG_Global[i] & (~G_STB);
              if (Bit > 0) REG_Global[i] = REG_Global[i] | G_STB;
            }
          }
          else if (strcmp(Register, "STB1") == 0)
          {
            sprintf(buf, "Set Global Register bit %s to %u.", Register, Bit);
            for (int i = 0; i < NCHIPS; i++)  {
              REG_Global[i] = REG_Global[i] & (~G_STB1);
              if (Bit > 0) REG_Global[i] = REG_Global[i] | G_STB1;
            }
            Serial.println(buf);
          }
          else if (strcmp(Register, "SLK") == 0)
          {
            ...
            Serial.println(buf);
            for (int i = 0; i < NCHIPS; i++)  {
              REG_Global[i] = REG_Global[i] & (~G_SLK);
